...
export class ReputexWidget {
  ...
  async fetchScore() {
    try {
      if (!this.validateDomain(this.userAddress.trim()) && !this.isAddress(this.userAddress.trim())) {
      ...
      if (scoreData.success && scoreData.data?.reputeXScore != undefined && scoreData.data?.reputeXScore != null) {
      ...
      <div>
        {this.visibility &&
          (this.showScoreComponent ? (
            <score-loaded reputeXScore={Math.trunc(this.score)} userAddress={this.userAddress} lastUpdated={`Last updated ${this.formatDate()}`} mode={this.mode}></score-loaded>
          ) : (
            <p>Failed Me</p>
